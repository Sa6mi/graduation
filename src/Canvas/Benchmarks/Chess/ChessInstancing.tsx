...
export function Chess(props: JSX.IntrinsicElements['group']) {
  ...
  const blackPawnTopRef = useRef<THREE.InstancedMesh>(null);

  React.useEffect(() => {
    // White pawns
    const whitePositions = [
      [0.218, -0.156], [0.156, -0.156], [0.094, -0.094], [0.031, -0.031],
      [-0.031, -0.156], [-0.094, -0.156], [-0.156, -0.156], [-0.219, -0.156]
    ];

    // Black pawns
    const blackPositions = [
      [-0.219, 0.156], [-0.157, 0.156], [-0.094, 0.156], [-0.031, 0.094],
      [0.031, 0.156], [0.094, 0.156], [0.156, 0.156], [0.219, 0.156]
    ];
    ...
    whitePositions.forEach((pos, i) => {
      matrix.setPosition(pos[0], 0.015, pos[1]);
      whitePawnBodyRef.current?.setMatrixAt(i, matrix);
      matrix.setPosition(pos[0] - 0.031, 0, pos[1] + 0.031);
      whitePawnTopRef.current?.setMatrixAt(i, matrix);
    });

    blackPositions.forEach((pos, i) => {
      matrix.setPosition(pos[0], 0.015, pos[1]);
      blackPawnBodyRef.current?.setMatrixAt(i, matrix);
      matrix.setPosition(pos[0] - 0.031, 0, pos[1] + 0.031);
      blackPawnTopRef.current?.setMatrixAt(i, matrix);
    });
  }, []);
  ...
  return (
      <group {...props} dispose={null}>
        ...
        />
        <instancedMesh
          ref={whitePawnBodyRef}
          args={[nodes.Pawn_Body_W1.geometry, materials.Pawn_Body_White, 8]}
          castShadow
          receiveShadow
        />
        <instancedMesh
          ref={whitePawnTopRef}
          args={[nodes.Pawn_Top_W1.geometry, materials.Pawn_Top_White, 8]}
          castShadow
          receiveShadow
        />
        <instancedMesh
          ref={blackPawnBodyRef}
          args={[nodes.Pawn_Body_W1.geometry, materials.Pawn_Body_Black, 8]}
          castShadow
          receiveShadow
        />
        <instancedMesh
          ref={blackPawnTopRef}
          args={[nodes.Pawn_Top_W1.geometry, materials.Pawn_Top_Black, 8]}
          castShadow
          receiveShadow
        />
